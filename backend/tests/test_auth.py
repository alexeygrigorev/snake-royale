...
def test_signup_returns_user_token_and_cookie(client: TestClient):
    response = client.post("/api/auth/signup", json={"username": "dana", "password": "secret"})
...
def test_signup_uses_secure_cookie_when_enabled(
    ...
    response = client.post("/api/auth/signup", json={"username": "evan", "password": "secret"})
...
def test_protected_route_without_token_returns_unauthorized(client: TestClient):
    ...
    response = client.post("/api/scores", json={"mode": "walls", "score": 10})
